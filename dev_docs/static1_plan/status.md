...
最新补充以roadmap 13.1—13.7为准。已按 E1 → F基础改进 → E2 完成这一轮实现：统一真实pipeline步骤事件与逐次usage缺失原因；安全停止和确认保存；主窗口整理；检索结果来源分散、必要子问题与综合推理提示；共享可配置预算并为最终回答预留；历史编辑分支、重命名、归档恢复及断开/重连。语音极简回答与独立rules仅列TODO，当前不实现。
...
- E1（已实现）：真实步骤、usage缺失原因、安全停止/保存、主窗口整理。
- F（结构性改进已实现，质量继续由真实失败问题驱动）：多来源召回分散、子问题拆解、阅读与模型预算、保留最终回答空间、多证据综合与有条件推断。
- E2（已实现）：编辑问题创建可恢复的新会话分支、重命名、归档恢复、断开与重连。
- 已知usage事实：新请求曾完整报告194 tokens（输入82、输出112）；分类超时请求保留38,433的部分用量并正确标为不完整，刷新后记录仍在。历史旧记录缺失不能补算；本轮新增的是逐次调用缺失原因。
- 本轮通过83项后端、12项前端检查、ruff与生产构建；浏览器实际检查了步骤收束、停止后编辑、分支保留和断开行为。QA效果不以这些检查替代真实提问。

...
本节作为A—D之后的当前路线。此前“代码与测试通过”只证明已验收的功能，不代表专业问答质量和真实使用体验已经达标。执行顺序固定为 E1 → F → E2 → G，完整会话管理不再阻塞专业问答改进。

### 13.1 反馈与现状

- QA：用户实际遇到信息召回不够、知识库缺直接答案、结合已有知识推理偏弱。分别定位为“材料是否存在—能否找到—是否读全—能否综合”，不能统一归因于模型或只扩大top-k。
- Token：已有请求级usage采集与显示。18766实际验证过完整用量194（输入82、输出112），也验证过分类超时后保留部分用量38,433、正确标记不完整并在刷新后保留。历史旧记录没有usage不能补算；当前缺口是逐次模型调用的缺失原因，而不是重新证明采集链路是否存在。
...
### 13.2 E1：先让执行可观察并安全收束（P0）

目标：用户能知道系统是否在工作、花费多少、是否已保存，并能安全停止。只纳入真实步骤事件、Token缺失原因、停止与保存、主窗口整理。

1. 在用户实际访问入口显示应用版本/运行标识，核对前后端版本、端口和持久化目录；用一条新请求复现Token缺失，避免拿历史空字段判断新版功能失效。
2. 答案上方用可折叠的“处理过程”展示真实pipeline步骤；收起时保留当前步骤/结束状态。具体布局与事件要求以13.5为准。
3. 保留首token / Thinking等待及总耗时一行；说明它衡量首段正文等待，不能冒充模型内部思考耗时。usage显示输入/输出/总量及完整性；区分历史未记录、供应商未返回、部分调用已报告、仍在统计。缺失不能显示为0，已有准确用量不能被后续空事件覆盖。
4. 保存中/已保存/失败清晰可见。停止、切换和退出统一遵循硬顺序：停止接收旧流 → 按该run标识收束回答版本 → 保存最终快照并确认成功 → 才允许切换、编辑或断开。旧请求的异常处理与清理必须核对run标识，不能修改新回答。
5. 正常结束和服务端异常由服务端记录终态；用户主动中断或网络断开由客户端标记“已中断／结果未确认”，保留最后收到的usage。未收到最终usage继续标为不完整，不承诺精确结算。

验收：同一实际入口的新请求显示usage或可解释的缺失原因；等待期间状态可见且可停止；重点检查停止后的步骤收束、最终快照保存以及旧流不会串入新回答。

### 13.3 F：专业问答质量闭环（P1，核心改进）

目标：充分利用现有材料，在没有逐字答案时仍能给出有依据的综合判断和建议。

1. 从用户当前遇到的具体失败对话入手，按需保留问题、预期、运行轨迹和改进前回答；不规定题量或建立场景矩阵。先定位问题所在，再修改并实际重问。
2. 召回改进：拆出必要子问题，按需进行中英术语/同义词改写；兼顾不同子问题与来源，避免结果被同一文档重复片段占满。先修复检索/阅读路径中的具体失败，再决定是否增加重排器。限定资料仍严格执行范围，不暗中扩展到全部知识库。
3. 阅读与预算：区分摘要命中和正文证据，按需沿章节、链接或相邻段继续阅读。由子问题缺口和新增信息决定补查；固定阅读限制改为可配置的有界预算，分别限制搜索、正文量、模型调用与总时长。快速路径和升级研究共享预算；模型调用预算覆盖分类、快速查证和内层研究，并为最终回答预留一次调用。研究达到预算后停止查证，利用已有证据交付部分答案；Token缺失时仍由调用次数、正文量和时间限制兜底。不能仅把top-k或轮数统一调大。
...
7. 领域笔记暂不扩展，不作为本轮QA改进前置条件；移除主对话中的“保存为领域笔记草稿”入口。已有笔记数据不随界面调整删除。

验收：逐例核对必要子问题覆盖、来源相关性/充分性、推断是否有依据与边界、答案是否有帮助；同时记录时间和实际usage。至少能区分召回改善与生成改善。缺资料案例应给出受支持部分及明确缺口，不为追求答全编造；没有可比用量时不下成本改善结论。

### 13.4 E2：完整会话与连接交互（P1）

- 历史入口常驻显示标题、更新时间和运行状态；提供新建、切换、重命名、归档/恢复与导出。归档不删除数据。
- 历史编辑第一版采用“复制为新会话分支”：原会话完整保留，新会话继承编辑位置之前的有效对话，提交修改后的问题，并记录来源会话与轮次。取消编辑不提交、不删除原版本。
- 增加断开与重新连接。断开必须等待E1的停止和确认保存顺序完成；保存失败时保留当前内容，不显示断开成功。断开只停止页面请求和轮询，不终止本地服务。

验收：停止后立即编辑不会混入旧事件；保存失败不会退出；原分支可以恢复；重命名、归档恢复、断开重连不丢失已保存内容。

### 13.5 G：体验整理与阶段收口（P2）

- 将资料管理、会话历史、运行详情分区，主界面保持问题、答案和当前状态清楚；技术细节收进可展开详情。
- 文本答案保持当前良好的布局与编排，不为了未来语音而压缩当前专业回答；语音专用规则只记录为TODO，见13.6。
...
执行顺序：E1先对齐真实步骤、运行状态与安全保存；随即进入F，不让完整历史管理延迟问答改进；再完成E2和G。沿用当前Python/React工程，按具体缺口借鉴WeKnora/chat-langchain，不迁移技术栈或从零重写。


#### 主对话窗口与交互优化（已实施，继续由实践校正）

按用户最新要求调整E/G；保留答案正文现有排版。建议单条回答自上而下：

1. **处理过程（可折叠）**：去掉主窗口“静知/静知助手”标题及逐条回答的“静知”标记，移除“保存为领域笔记草稿”按钮。将现有静态策略串移入折叠详情，收起时只显示当前步骤或最终状态。展开后按实际执行顺序展示 step 1、step 2……，例如理解问题、搜索某关键词、阅读某文档、继续补查、综合答案；没有执行的步骤不展示为已执行。
2. **答案正文**：保留当前Markdown布局、段落、列表和代码样式。
3. **耗时与操作**：保留首token/Thinking等待、总时间和Token信息；“复制答案”“重新生成”放在同一条紧凑操作栏，按钮视觉统一。完整运行记录继续可展开查看，避免把技术细节摊满页面。
4. **已读证据**：按正文引用编号展示可换行的链接，如 `[1] 标题`、`[2] 标题`、`[3] 标题`；展开或点开再看版本、原文与摘录。相同原文入口避免重复铺陈；展示调整不能打乱正文引用编号。

上方“处理过程”负责说明做了什么，下方“运行记录”负责耗时、调用与用量，两者读取同一份运行记录，不各自推断一套状态。

**底层pipeline对齐（允许必要的框架调整）：**
- 从实际节点、模型调用、工具执行处产生结构化步骤事件；包含运行/步骤标识、顺序、所属阶段、开始/结束/停止/失败状态，以及能拿到的时间、来源或调用用量。前端根据事件呈现，不解析中文status文案拼步骤。
- 快速查证升级、缓存复用、补查、版本核验、最终输出需如实区分；模型调用可映射到所属步骤。过程说明展示操作和结果摘要，不展示模型私有思维链，也不伪造“思考步骤”。
- 取消、异常与正常完成都要收束当前步骤。历史保存同一运行记录，重新生成及编辑重问使用新的运行标识，旧流事件不能串入新回答。
- 检查现有阶段耗时与完整请求时间之间的差额，按实际调用边界补齐建图/初始化/等待等记录；客户端首正文等待与服务端节点耗时保留各自口径，不凑数。usage缺失仍为未知。
- 允许调整现有graph、工具封装和SSE协议来统一状态来源；目标是消除根因，不另造复杂通用工作流平台。

**输入区设置：**
- 保留回答范围、证据要求、查证方式、资料范围四项，统一控件高度、间距、文字层级与响应式换行；“设置用于新问题”降为辅助说明。
- 资料范围仍是实际检索/读取约束。UI美化不改变其含义。

**历史问题复制、编辑与重问：**
- 已发送问题新增“复制”“编辑”操作。编辑时加载原问题；若当前仍在生成，先中断并停留在编辑状态，不自动发起请求。
- 确认修改后自动重新询问，默认沿用被编辑轮的有效配置，并清楚展示；只把该问题之前的有效对话作为上下文，不带旧答案及其后续对话。
- 修改历史问题后，原问题、旧回答和后续对话保留为可恢复的原版本；新结果形成当前分支。先实现满足此行为的最小版本保存，不建设复杂分支可视化系统。
- 编辑支持取消/退出；取消不提交问题、不删除历史，也不自动恢复已中断的生成。切换历史编辑对象时保留草稿或明确提示未提交状态，避免静默丢失。

**断开连接与状态联动：**
- 网页提供“断开连接/退出”及“重新连接”。退出时先停止当前问答并保存历史，再进入主动断开状态，停止轮询和自动重连；断开后禁用发送和需服务的知识库操作，保留已加载历史可读。
- 保存失败不能静默显示退出成功；保留内容，提供重试或导出。再次连接恢复正常状态与历史，不新建空白数据目录。
- 区分应用服务连接、知识库准备状态、LLM配置/最近调用结果；左下角LLM状态提高视觉显著度，同时使用文字，不能仅凭API key存在显示“模型连接正常”。
- “断开连接”不等于关闭后台服务；关闭本地服务器不属于本次网页退出的默认行为。

### 13.6 static2语音专用rules（仅TODO，当前不实现）

语音交流需要独立的任务规则，而不是直接朗读文本专业问答。未来单独定义voice rules，至少包含答案长度（极简、短句、一次一个要点）、答案风格（适合口语）、语气、默认evidence_level（考虑low）、追问方式、响应节奏与被打断后的行为；必要时修正语音任务目标和约束，以交流顺畅为核心。

low不意味着编造事实，用户指定资料与实际来源边界仍须遵守。具体规则在语音阶段通过实际交流确定；本轮不实现语音规则引擎、模式开关或语音接口，也不为此改写static1文本答案策略。

### 13.7 实施方式与本轮结果：源头修正、实践驱动

- 遇到问题先沿事件来源、状态生命周期、检索/阅读/生成或持久化链路定位根因，再修改负责该行为的模块；避免靠前端猜测、堆提示词或增加例外分支掩盖底层问题。
- 不扩充固定数量的验证案例，不引入额外评估Agent、大型测试矩阵或每轮全面重复验证。保留必要构建/类型检查；涉及历史丢失、取消串流等实际边界时做针对性检查即可。
- 完成可用改动后交由实际对话验证，记录暴露的问题并继续修正。当前没有失败证据的潜在扩展不提前建设。

#### 本轮实现结果（2026-09-17）

- E1：SSE新增run_id与结构化step事件，实际节点、搜索和阅读工具产生开始/完成/失败状态；usage记录每次调用的phase及缺失原因。前端按run隔离事件，中断后收束运行步骤，并串行等待最终快照保存。
- F：检索第一页优先分散来源；研究提示拆分必要子问题、术语改写与多来源覆盖；最终回答明确综合互补证据、区分推断依据/条件/缺口。新增可配置阅读、模型调用和研究时限，并为最终回答预留调用。以上是结构性改进，实际QA充分性仍按真实失败问题继续修正。
- E2：问题复制与编辑、编辑后创建新会话分支、来源会话/轮次记录、会话重命名、归档恢复、断开/重连已实现。停止后立即编辑和断开都等待旧流收束及保存；保存失败不会继续切换。
- G/UI：移除主窗口“静知”和领域笔记草稿入口；保留答案正文排版；处理过程、运行记录、复制/重新生成、编号证据及设置区重新编排；服务、知识库和LLM状态分开显示。
- 轻量检查：83项后端测试、12项前端检查、ruff和生产构建通过。浏览器实际验证了停止后编辑、原分支保留、新分支持久化及断开；专业问答质量不以测试数量宣称达标。

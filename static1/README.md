...
每条已结束的答案可一键复制原始 Markdown；最新一轮可“重新生成”，复用原问题与原请求上下文，旧版本保留在“之前的回答”。停止或失败后的部分正文也可复制；未完成的新版本不会加入后续提问上下文。重新生成不清空输入框草稿。

“首 token / Think 等待”指发送到浏览器收到第一段非空正文的时间，包含检索、阅读及网络等待，不是模型内部推理时间。“总时间”计到成功完成事件；停止/失败仅显示已耗时和未完成状态。运行时实时刷新，结束后固定。导出包含每个版本的耗时与状态，未获得的数值保留null；刷新页面仍会丢失对话。

前端源码变更后须重新构建；本次已执行生产构建。若浏览器还显示旧页面，请刷新。token用量、缓存命中和详细步骤历史尚未实现。
...
默认最多两轮研究、6次不同查询、6段已读证据，每轮图步数24，总超时180秒。finish_research交接子问题、证据ID、缺口与下一动作；补查不会丢掉旧的未解决子问题。无新增证据的重复轮停止，输出策略问题直接交给回答阶段。

...
打开 http://127.0.0.1:8000 ，在输入框上方选择回答范围、证据要求与可选文档。默认自动判断、有据分析：闲聊和一般概念可直接回答，具体资料问题进入原有研究流程。选择“仅按资料”或限定文档时，即使使用“自由讨论”也不会扩大资料范围。无法识别“这份资料”时会请你明确选择。

每条答案显示服务端实际执行的策略。重新生成沿用该版本的生效配置；下拉框的新设置用于下一条问题。旧版本保留各自的配置、来源和耗时，导出可查看这些字段。

知识库加载中或失败时仍可发送一般交流，资料查证会说明不可用。配置密钥不代表模型已验证可用。默认配置可通过 `QUERY_ROUTING=auto|knowledge_only`、`EVIDENCE_LEVEL=low|middle|high` 设置；原 `EVIDENCE_ROUTING` 仅控制研究内部补查逻辑。限定文档当前使用 BM25，全库仍可使用混合检索。
...
前端加载期间展示状态卡片与向量片段进度。服务已连接且配置密钥即可发送，知识库就绪单独决定能否查证。失败与断连分别提示，不清空输入。索引没有可用总数时不显示虚构百分比。浏览器无法在端口被其他程序占用时展示本应用错误页，因为请求尚未到达本应用；此类错误在启动终端说明。

## 处理过程、答案操作与耗时

每条回答的“处理过程”来自服务端实际执行事件，可展开查看理解、搜索、阅读、核验和回答步骤；不会用状态文案或模型文本猜测步骤。每条已结束的答案可复制原始 Markdown；最新一轮可重新生成。已发送问题可复制或编辑，编辑确认后复制为新会话分支，原会话完整保留。

“首 token / Think 等待”指发送到浏览器收到第一段非空正文的时间，包含检索、阅读及网络等待，不是模型内部推理时间。“总时间”计到成功完成事件；停止/失败仅显示已耗时和未完成状态。usage显示供应商已报告的输入、输出和总量；缺失时保留逐次调用原因，不按0结算。用户中断或网络断开标记“结果未确认”，保留最后收到的部分用量。

会话保存在 `data/workspace.sqlite3`。切换、编辑或断开连接遵循“停止旧流、收束回答、确认保存、再切换”的顺序；保存失败时不显示退出成功。归档不会删除会话，重新连接可恢复服务轮询。前端源码变更后须重新构建；若浏览器还显示旧页面，请刷新。
...
用户 → FastAPI → Deep Agent(search_docs → read_doc)
                     ↓
                LangGraph validate
                     ├─ 子问题未覆盖且可补查、预算允许 → 定向补查
                     ├─ 目录确认缺页且不能补页 → 简短说明与单步下一动作
                     └─ 覆盖完成或达到停止条件 → answer → SSE
```
...
默认最多两轮研究、6次不同查询、8段已读证据、12次模型调用，每轮图步数24，研究阶段最多120秒，总超时180秒。预算由快速查证和深入研究共享，最后一次模型调用为最终回答预留；研究达到预算或超时时停止补查，用已有证据交付部分答案。finish_research交接子问题、证据ID、缺口与下一动作；补查不会丢掉旧的未解决子问题。无新增证据的重复轮停止，输出策略问题直接交给回答阶段。
validate核验版本、正文及报告证据ID，并按报告路由，**不等于独立的语义充分性裁判**。实际质量仍需真实模型评估。EVIDENCE_ROUTING=false可切回旧路由；MAX_SEARCHES控制新路由查询上限。

问答期间只读本地语料，不自动联网补页。check_corpus_page仅核对用户或已读正文中出现的官方URL；确认未入库时硬停止并给出一个补材料动作，不把搜索未命中当作缺页。配置修改后需重启服务。

## 存储与范围
- data/knowledge.sqlite3：原文页、标题、来源、parser、抓取时间、内容版本；不保存 Cookie。
- 来源链接带版本；来源更新后旧版本链接会明确报错，不静默展示新版作为旧证据。
- 当前仅保留每个来源的最新版本，无跨版本档案。
- 对话、回答版本、生效配置、步骤、用量和来源保存到独立的 workspace.sqlite3；未完成回答不加入后续上下文。
- 编辑历史问题会创建新会话分支，并记录来源会话与轮次；原会话可继续恢复。

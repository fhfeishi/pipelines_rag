...
Usage:
    uv run -m rag_pdfs.ingest_img tmp/raws/foo.pdf --out tmp/outs/foo --dry-run
    uv run -m rag_pdfs.ingest_img tmp/raws/foo.pdf --out tmp/outs/foo --build-chroma
    uv run -m rag_pdfs.ingest_img --batch --build-chroma
    uv run -m rag_pdfs.ingest_img tmp/raws/foo.pdf --out tmp/outs/foo --skip-parse --resume
"""
...
from rag_pdfs.caption_chunks import (
...
from rag_pdfs.model_runtime import ModelRole
from rag_pdfs.pdf_filter import (
...
# Caption 质量校验（plan.md Phase 2）：只标记不重试——vision LLM temperature=0，
# 同 prompt 重试大概率得到同样输出；低质量项靠人工 spot-check 或换模型重跑。
...
def assess_caption_quality(caption_text: str) -> str:
    """返回低质量原因；空串 = 合格。结果写入 ImageCaption.quality_flag。"""
    text = caption_text.strip().lower()
    if not text:
        return "empty"
    if len(text) < MIN_CAPTION_CHARS:
        return f"too_short(<{MIN_CAPTION_CHARS} chars)"
    hits = [marker for marker in LOW_QUALITY_MARKERS if marker in text]
    if hits:
        return "uncertainty_markers: " + ", ".join(hits)
    return ""
...
def build_captions(
    *,
    ...
    vision_llm_factory: Callable[[], Any] | None = None,
    ...
    for element in image_elements:
        ...
        if element.index in cache and not is_placeholder_caption(cache[element.index]):
            cached = cache[element.index]
            if not cached.quality_flag:
                flag = assess_caption_quality(cached.caption)
                if flag:
                    cached = cached.model_copy(update={"quality_flag": flag})
            captions.append(cached)
            print(f"[cached] {cached.image_id} page={element.page}")
            continue
        ...
        if dry_run:
            ...
            if not llm:
                if vision_llm_factory is not None:
                    llm = vision_llm_factory()
                else:
                    if not api_key:
                        raise RuntimeError(
                            "Missing vision API key. Pass --api-key or set the "
                            "provider key in configs/.env."
                        )
        ...
        quality_flag = "" if dry_run else assess_caption_quality(caption)
        captions.append(
            ImageCaption(
                image_id=image_id,
                page=element.page,
        ...
        marker = f" [low-quality: {quality_flag}]" if quality_flag else ""
        print(
            f"[captioned] {image_id} page={element.page} "
            f"{stored_path or image_path}{marker}"
        )

    low_quality = [caption for caption in captions if caption.quality_flag]
    if low_quality:
        print(
            f"[warn] {len(low_quality)}/{len(captions)} captions flagged low-quality: "
            + ", ".join(caption.image_id for caption in low_quality)
        )
    ...
    stats = {
        ...
        "dry_run": dry_run,
        "vision_model": model,
        "low_quality_captions": len(low_quality),
    }
...
def build_embeddings(model_path: str | None) -> Any:
    ...
    return RAGRuntime().embeddings(model_path=model_path).instance
...
def build_chroma_indexes(
    *,
    ...
    embedding_handle = active_runtime.embeddings(model_path=embedding_model_path)
    embeddings = embedding_handle.instance
    chroma_root = out_dir / "chroma"
    chroma_root.mkdir(parents=True, exist_ok=True)
    manifest_path(out_dir).unlink(missing_ok=True)
    ...
    for doc_key, collection_name in collections.items():
        ...
        print(
            f"[chroma] {collection_name}: {count} docs -> {persist_dir}"
        )

    manifest = write_index_manifest(
        out_dir,
        embedding_handle.spec,
        collection_counts,
    )
    print(
        f"[manifest] embedding={embedding_handle.spec.id} "
        f"fingerprint={manifest.embedding.fingerprint} -> {manifest_path(out_dir)}"
    )
    return manifest
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override vision request timeout; catalog/default is used when omitted",
    )
...
def ingest_one_pdf(
    ...
    active_runtime = runtime or RAGRuntime.from_model_config(args.model_config)
    ...
    preset = VISION_PRESETS[args.vision_provider]
    vision_handle = active_runtime.chat(
        ModelRole.VISION_INGEST,
        api_key=args.api_key,
        base_url=args.base_url,
        model=args.model,
        timeout=args.timeout,
        temperature=None if active_runtime.uses_catalog else 0,
        max_tokens=None if active_runtime.uses_catalog else 1200,
        default_api_key_env=preset["api_key_setting"].upper(),
        default_base_url=preset["base_url"],
        default_model=preset["model"],
        capabilities=("text", "vision"),
    )
    vision_provider = (
        f"catalog:{vision_handle.spec.id}"
        if active_runtime.uses_catalog
        else args.vision_provider
    )
    print(
        f"[vision] provider={vision_provider} model={vision_handle.spec.model} "
        f"base_url={vision_handle.spec.base_url or '(local)'}"
    )
    ...
    captions, skipped, caption_stats = build_captions(
        ...
        out_dir=out_dir,
        api_key=None,
        base_url=vision_handle.spec.base_url or "",
        model=vision_handle.spec.model,
        before_chars=args.before_chars,
        after_chars=args.after_chars,
        timeout=int(vision_handle.spec.timeout),
        ...
        vision_llm_factory=lambda: vision_handle.instance,
    )
...
def main() -> None:
    ...
    if args.batch:
        ...
        for pdf_path in pdfs:
            ...
            results.append(ingest_one_pdf(pdf_path, out_dir, args, runtime))
    ...
    ingest_one_pdf(pdf_path, out_dir, args, runtime)

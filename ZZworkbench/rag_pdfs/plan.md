...
## 0.4 顶层架构与模型运行时（2026-08-29）

RAG 产品的顶层设计、成熟项目调研、模型目录、LangGraph 工作流、LangSmith 可观测性与分阶段迁移方案，统一记录在 rag_pdfs/architecture.md。

本轮已完成 M0 基础设施：

- model_runtime.py：用 kind、role、runtime、source 四个正交维度描述模型，并统一 API、本地目录、Hugging Face 缓存与 ModelScope 缓存实例化。
- prompts.py：集中管理回答、查询改写和历史摘要提示词及其版本。
- configs/models.example.toml：提供可复制的模型角色配置。

M1 已于 2026-08-29 完成：新增 runtime.py 组合根，三条 CLI 支持 --model-config/RAG_MODEL_CONFIG，同时保留原 API、本地路径参数作为覆盖；Chroma 构建写入 index_manifest.json，query/eval 校验 embedding fingerprint；query 的 evidence、调试分数和回答上下文复用同一次检索。legacy 索引默认告警兼容，--strict-index-manifest 可强制拒绝。
...
| `model_runtime.py` | 可用 | provider adapter、模型目录、role/capability 与 embedding fingerprint |
| `runtime.py` | 可用 | 三条 CLI 的 composition root；catalog 与 legacy override |
| `index_manifest.py` | 可用 | 写入、读取并校验持久化索引的 embedding contract |
| `embeddings.py` | 留在 `rag_langchain/` | 本地 Qwen3 embedding 历史探索；主线统一走 `runtime.py` |

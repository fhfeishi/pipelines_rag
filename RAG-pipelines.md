...
> 回答的问题：企业私有知识问答、技术文档问答、代码库问答、结构化业务数据问答和多模态 PDF RAG 应如何选择 pipeline。
> 适用场景：从零设计私有知识问答架构，或判断某个问题是否真的需要重型 RAG。
> 不回答：具体厂商模型价格、完整平台实现、某个向量库或数据库的选型。
> 当前结论：RAG 没有退场，但它已经从默认答案变成按场景选择的工程组件。
> 可信度：本地思考整理；具体模型窗口、缓存价格、工具能力需要按当前官方文档核验。
> 主要来源：用户粘贴文本、[[04-技术文档图片如何进入 RAG]]、[[02-实验设计：图片 caption 是否改善 RAG]]。

## 核心判断

2026 年的私有知识问答不适合默认从“切 chunk、建向量库、Top-K 检索”开始。更准确的行业状态是：

> RAG 没有退场，但它已经从默认答案变成了需要按场景选择的工程组件。

长上下文、Prompt Caching、Agentic Retrieval、Text-to-SQL 和 Deep Research 都已经成熟到足以改变架构选型。工程上应先判断：
...
| Claude 1M / Gemini 2M 可全量塞上下文 | 部分成立 | 长上下文能力必须按模型、API 与套餐确认。名义窗口不等于可靠可用窗口。 |
| Prompt Caching 二次调用成本可大幅降低 | 基本成立但需限定 | 降本取决于缓存命中率、稳定前缀、TTL、模型价格和动态内容位置。 |
| Cursor 等代码工具正在弱化索引，转向直接抓取 | 需要谨慎 | 更准确说法是代码场景正在走向“语义索引 + grep/regex + 文件读取 + agent loop”的混合搜索。 |
| Deep Research 证明传统 RAG 被淘汰 | 过度推断 | Deep Research 证明 planner/reader/aggregator 式 agent research 很强，但其中通常仍包含检索、阅读、筛选和证据聚合。 |
| Baseline RAG 禁用于生产 | 方向正确，表述过绝对 | 简单切片 + embedding + Top-K 可用于原型和低风险场景；生产知识问答通常需要 hybrid search、rerank、metadata filter、citation、eval 和监控。 |
| Reranker 是最高性价比升级 | 方向正确，数字不可泛化 | Reranker 常常有效，尤其适合从较大候选集中筛出高相关证据；实际收益必须由具体 eval 证明。 |
| 结构化数据应走 Text-to-SQL/API，而非向量化 | 成立 | CRM、ERP、订单、库存、指标等结构化数据，SQL/API 工具调用通常比向量库更正确、更可审计。 |
| Eval 比换模型重要 | 成立 | 没有评估闭环的 prompt 调参不可控。RAG/Agent eval、trace 和反馈闭环应作为生产系统基础设施。 |
...
优势是工程简单、保留完整语境、缓存命中时成本和延迟更低。风险是长上下文仍可能出现位置偏差、遗漏和多跳推理衰减；强权限隔离、多租户、高频更新场景也不适合简单全量塞入。
...
代码场景尤其适合 agentic retrieval，因为函数名、类名、import path、错误码天然适合精确搜索；agent 可以先 grep 定位，再读完整文件，而不是只读碎片 chunk。

风险是成本和延迟高，弱模型容易循环、误判工具结果或过早停止。生产中必须设置 step limit、工具白名单、trace 和失败退出策略。

### 4. Text-to-SQL / Tool-to-API
...
这正是本 solution 的 image ingestion 轨道需要继续推进的核心实验。
...
很多私有知识问答失败不是因为模型不够强，而是数据进入 pipeline 前已经损坏。
...
当前专题的核心实验是：图片 caption 应该 inline 进入正文，还是作为独立 `image_caption` evidence chunk。
...
- [[04-技术文档图片如何进入 RAG]]
- [[01-caption 存储：inline 与 separate chunk]]
- [[02-实验设计：图片 caption 是否改善 RAG]]
- [[99-资料来源与待核验]]
